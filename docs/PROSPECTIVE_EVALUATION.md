...
pre-event forecasts and **verify** saved batches. A separate
`run_frozen_replay` command can now fit the provisional recent + Elo logistic
research model on the accepted snapshot; the archive replays probabilities
for that named model. See `FROZEN_PROSPECTIVE_MODEL.md`. The general archive
does not train a model, fetch a live card, infer fighter links, compute
pre-fight features, or score results. No real future prediction has been
recorded yet. The latest accepted
...
full card. The recent + Elo model is a working research reference, not a
chosen live release.
...
| `--model-spec` | JSON object with model name, 40-character training-code commit, training-through date, training-input SHA-256, model artifact SHA-256, and the ordered feature-column list. The original four baseline/defense/outcome schemas and the 36-column recent + Elo schema are accepted. |
| `--model-artifact` | Nonempty trained model file whose bytes match the specification. For the named recent + Elo JSON artifact, numeric parameters are parsed and replayed; older artifact types remain opaque prototype receipts. |
...
conditions. It also replays the named numeric JSON model. Older artifacts
are never loaded. Verification fails after a copied file is changed or removed.
...
binary for legacy model names. For `symmetric_recent_elo_json_v1`, saved
numeric parameters are validated and every probability is replayed during
both record and verify. That does not establish the historical availability
or source accuracy of manually submitted features.
...
forecasts. Current-data ingestion and audited as-of feature generation come
next; this archive is a tested storage gate awaiting those inputs.
