"""Test paired fighter values with one fixed tree on the old development folds."""
...
def main() -> None:
    ...
    for name, default in {
        ...
        "output": base / "experiments/paired_context_v1",
    }.items():
    ...
    print(
        f"Individual pre-fight rows checked: {report['individual_feature_rows_checked']}"
    )
    print("All 12 reference probability series: preserved exactly")
    print("Matched reference tree: refit agreed within 1e-12 in each fold")
    print(
    ...
    for label, scores in [
        ...
        *((fold["name"], fold["scores"]) for fold in report["folds"]),
        ...
        print(f"\n{label}")
        print(
            f"{'Variant':36} {'Correct':>9} {'Accuracy':>9} {'Log loss':>10} {'Brier':>10}"
        )
        for name in (LOGISTIC_REFERENCE, COMPARATOR, CANDIDATE):
            score = scores[name]
            count = f"{score['correct_bouts']}/{score['decisive_bouts']}"
            print(
                f"{name:36} {count:>9} {score['accuracy']:>8.2%} {score['log_loss']:>10.6f} {score['brier_score']:>10.6f}"
            )
    print("\nPaired differences: new minus reference; accuracy in percentage points")
    for name, pair in report["paired_comparisons"].items():
        ...
        print(
            f"{name}: accuracy {100 * pair['delta']['accuracy']:+.2f} [{100 * a:+.2f}, {100 * b:+.2f}]; log loss {pair['delta']['log_loss']:+.6f} [{lo:+.6f}, {hi:+.6f}]"
        )
    ...
    print(
        "AUC, folds, subgroups, symmetry, observed training columns and hashes are in the manifest."
    )
